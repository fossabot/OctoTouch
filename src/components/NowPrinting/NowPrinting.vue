...
<template>
  <div fill-height fluid class="now-printing__container">
    ...
    <v-row class="now-printing__details">
      ...
      <v-col class="now-printing__progress-below now-printing__progress-details" style="color: white; font-size: 40px; line-height: 20px;" :sm="10" v-ripple align="end">
        ...
        <h3 class="now-printing__details-subtitle">
          ...
          <span v-if="job.filamentUsage.length > 1">{{job.filamentUsage[1]}}g <v-icon color=white>mdi-printer-3d-nozzle</v-icon></span>
        </h3>
      </v-col>
    </v-row>
    ...
    <div class="now-printing__details" v-if="screen == 'printing'">
      ...
      <v-row class="now-printing__overview">
        <v-col @click="confirmCancel()" v-ripple class="now-printing__overview-item">
        ...
        <v-col @click="openPrintUtilties()" v-ripple class="now-printing__overview-item">
          <v-icon size=15vh color=#dfe6e9>mdi-cog</v-icon><br>
        </v-col>
      </v-row>
    </div>

    <div class="now-printing__details" v-if="screen == 'utilties'">
      <v-row class="now-printing__overview">
        ...
        <v-col @click="screen = 'z-offset'" v-ripple class="now-printing__overview-item">
          <v-icon size=15vh color=#dfe6e9>mdi-pencil-ruler</v-icon><br>
          <span style="color: #dfe6e9;">Adjust Z-Offset</span>
        </v-col>
    ...
    <div class="now-printing__details" v-if="screen == 'dialog'">
      DIALOG
    </div>
  </div>
</template>
  ...
  export default Vue.extend({
    ...
    methods: {
      ...
      confirmCancel: function() {
        this.showCancellationDialog().then((confirm) => {
          if(confirm == 1) {
            this.cancelPrint().then((data) => {
              this.update()
            })
          }
        })
      ...
      openPrintUtilties: function() {
        this.screen = "utilties"
      }
    },
    computed: {
